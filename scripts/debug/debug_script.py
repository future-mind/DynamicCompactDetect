...
def parse_args():
    ...
    parser.add_argument('--source', type=str, default='datasets/coco/val2017/000000119445.jpg', help='source')
...
def main():
    ...
    if pred.dim() == 4:
        ...
        bins = [0.0, 0.0001, 0.001, 0.01, 0.1, 0.25, 0.5, 0.9]
        ...
        for i in range(len(bins)):
            if i < len(bins) - 1:
                print(f"  Scores {bins[i]:.6f} to {bins[i+1]:.6f}: {counts[i].sum().item()}")
            else:
                print(f"  Scores > {bins[i]:.6f}: {counts[i].sum().item()}")
