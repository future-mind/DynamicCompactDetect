...
def train_one_epoch(model, dataloader, optimizer, scheduler, loss_fn, epoch, device, cfg, scaler=None, ema=None, writer=None):
    ...
    # Set up gradient clipping if enabled
    clip_grad = cfg['training'].get('clip_gradients', 0.0)
    ...
    for batch_idx, (images, targets) in enumerate(dataloader):
        ...
        # Handle targets based on type (list or dict)
        if isinstance(targets, list):
            # If targets is a list of tensors, move each to device
            targets = [t.to(device) for t in targets]
        elif isinstance(targets, dict):
            # If targets is a dictionary, move each value to device
            for k in targets:
        ...
        if isinstance(loss_items, dict):
            box_losses.update(loss_items.get('box', 0), images.size(0))
            obj_losses.update(loss_items.get('obj', 0), images.size(0))
            cls_losses.update(loss_items.get('cls', 0), images.size(0))
        else:
            box_losses.update(loss_items[0], images.size(0))
            obj_losses.update(loss_items[1], images.size(0))
            cls_losses.update(loss_items[2], images.size(0))
        ...
        if batch_idx % cfg['logging']['log_interval'] == 0:
            ...
            if writer is not None:
                ...
                if cfg['logging'].get('log_images', False) and batch_idx == 0:
    ...
    try:
        ...
        if cfg['checkpoints']['save_last']:
            ...
            save_checkpoint(
                ...
                best_map=best_mAP,
                history=history, 
                path=os.path.join(cfg['checkpoints']['save_dir'], 'last.pth')
            )
