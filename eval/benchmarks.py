...
def export_results_to_csv(results, filename='results/benchmark_results.csv'):
    ...
    if 'input_sizes' in results:
        for size_key, size_metrics in results['input_sizes'].items():
            row = {'model': f"input_size_{size_key}"}
            # Extract early exit enabled metrics
            if 'early_exit_enabled' in size_metrics:
                for metric_name, metric_value in size_metrics['early_exit_enabled'].items():
                    row[f"ee_enabled_{metric_name}"] = metric_value
            
            # Extract early exit disabled metrics
            if 'early_exit_disabled' in size_metrics:
                for metric_name, metric_value in size_metrics['early_exit_disabled'].items():
                    row[f"ee_disabled_{metric_name}"] = metric_value
            
            # Add other metrics
            for metric_name, metric_value in size_metrics.items():
                if metric_name not in ['early_exit_enabled', 'early_exit_disabled']:
    ...
    if 'variations' in results:
        for model_name, metrics in results['variations'].items():
    ...
    if 'memory_usage' in results:
        for size_key, metrics in results['memory_usage'].items():
            row = {'model': f"memory_{size_key}"}
            for metric_name, metric_value in metrics.items():
                row[metric_name] = metric_value
            csv_data.append(row)
    ...
    if 'model_info' in results:
        row = {'model': 'model_info'}
        for metric_name, metric_value in results['model_info'].items():
            row[metric_name] = metric_value
        csv_data.append(row)
    
    # Write to CSV
    if csv_data:
        # Get all possible field names across all rows
        all_fieldnames = set()
        for row in csv_data:
            all_fieldnames.update(row.keys())
        
        fieldnames = ['model'] + sorted(list(all_fieldnames - {'model'}))
